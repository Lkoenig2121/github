...
export default function RepositoryDetailPage() {
  ...
  const fetchFiles = async () => {
    try {
      const response = await fetch(`http://localhost:3001/api/repositories/${repoId}/files`);
      const data = await response.json();
      setFiles(data);
    } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/repositories" className="text-indigo-600 hover:text-indigo-700 mt-4 inline-block">
